...
async  def _download_test_notebooks(workspace_id: str) -> None:
    ...
    to_download = [item async for item in workspace_client.get_items() if item.type == "Notebook" and item.display_name.startswith("test_")]

    for item in to_download:
        item_definition = await workspace_client.get_item_definition(item.id, "ipynb")
        with open(f"tests/{item.display_name}.ipynb", "w") as file:
